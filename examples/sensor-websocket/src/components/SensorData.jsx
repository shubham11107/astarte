...
function ListItem(props) {
  return (
    <ListGroup.Item
      key={props.index}
      className="temperature-list px-3 border-0 pb-0 py-2 font-weight-normal bg-transparent"
    >
      <b className="font-weight-bold">{props.label}: </b>
...
const setSensorDataCard = (index, sensor) => {
  ...
  return (
    <Row key={index} className="m-3 bg-light px-2 pt-3 pb-2">
      <Col className="text-dark px-3 py-2 bg-transparent border-0 font-weight-bold w-100 text-left btn text-decoration-none">
        <span className="bg-sensor-theme font-weight-light py-2 px-3 w-100 text-white d-inline-block">
          <strong className="font-weight-bold">Sensor Name : &nbsp;</strong>
          {sensor.name}
        </span>
        <Col className="p-0">
