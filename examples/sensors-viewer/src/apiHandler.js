...
function handleDeviceDataInterfaces(data) {
    ...
    const availableIndex = interfaces.findIndex(key => key.search(constant.AVAILABLE_SENSORS) > -1);
    const valueIndex = interfaces.findIndex(key => key.search(constant.VALUES) > -1);
    const samplingRateIndex = interfaces.findIndex(key => key.search(constant.SAMPLING_RATE) > -1);
...
export function getInterfaceById(device_id, interface_id, params = {}) {
    const URL = getAPIUrl('interface_by_id', {"device_id": device_id, "interface": interface_id});
    return GET(URL, params)
...
export function getInterfaceByAlias(device_alias, interface_id, params = {}) {
    const URL = getAPIUrl('interface_by_id', {"device_alias": device_alias, "interface": interface_id});
    return GET(URL, params)
        .then(response => response.data.data)
}
