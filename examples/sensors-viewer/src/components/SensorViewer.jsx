    ...
    setDeviceDataById = (id, params = {}) => {
        getDeviceDataById(id, params)
            .then(res => {
                    ...
                    getInterfaceById(id, res.interfaces[res.availableIndex])
                        .then(response => {
                            this.setState({availableSensors: response});
                        });
                    getInterfaceById(id, res.interfaces[res.valueIndex])
                        .then(response => {
                            this.setState({sensorValues: response})
                        });
                    getInterfaceById(id, res.interfaces[res.samplingRateIndex])
                        .then(response => {
    ...
    setDeviceDataByAlias = (alias, params = {}) => {
        getDeviceDataByAlias(alias, params)
            .then(res => {
                    ...
                    getInterfaceByAlias(alias, res.interfaces[res.availableIndex])
                        .then(response => {
                            this.setState({availableSensors: response});
                        });
                    getInterfaceByAlias(alias, res.interfaces[res.valueIndex])
                        .then(response => {
                            this.setState({sensorValues: response})
                        });
                    getInterfaceByAlias(alias, res.interfaces[res.samplingRateIndex])
                        .then(response => {
                            this.setState({sensorSamplingRate: response})
                        })
                }
    ...
    render() {
        ...
        return (
            <Container className="px-0 py-4" fluid>
                ...
                font-weight-bold text-white bg-success mb-5 px-0 py-3">
                                Sensor Viewer
                            </h5>
                                ...
                                <Row className="card-main-row mt-5">
                                    <Col xs={12} className="device-status-div px-3 pb-4">
                                        ...
                                        <h6 className="m-0 font-weight-bold position-relative">
                                         <span className="status-tag rounded-circle d-inline-block mr-2"
                                               style={{backgroundColor: `${data.connected ? '#008000' : '#ff0000'}`}}/>
                                            Device {data.connected ? 'Connected' : 'Disconnected'}
                                        </h6> : ""}
                                    </Col>
                                    {(!_.isEmpty(availableSensors) && !_.isEmpty(sensorValues) && !_.isEmpty(sensorSamplingRate)) ?
                                    <Col xs={12}>
                                        <Accordion>
                                            <SensorItem availableSensors={availableSensors} sensorValues={sensorValues}
                ...
                <CredentialsModal handleCredentialModal={() => this.handleCredentialModal(false)} visible={visible}/>
            </Container>
        );
    }
}
